...
from . import utility
from rex_gym.agents.scripts import utility as ut
...
class PPOAlgorithm(object):
    ...
    def __init__(self, batch_env, step, is_training, should_log, config):
        ...
        if fine_tuning:
          ...
          action_size = len(action_space[0])
          with tf.Session() as sess:
            saver = ut.define_saver(exclude=(r'.*_temporary/.*',))
    ...
    """
        ...
        # saver = ut.define_saver(exclude=(r"temporary/.*",))
        with tf.Session() as sess:
          saver = ut.define_saver()
          saver.restore(sess, checkpoint)
    ...
    def _define_end_episode(self, agent_indices):
        ...
        episode_end = datetime.now() - self._episode_start
        print('EPISODE TIME LENGTH : ' + str(episode_end.total_seconds()))
        with tf.control_dependencies([append]):
    ...
    def _update_meta_value(self, observ, reward, length):
        """Perform multiple update steps of the value baseline.
        ...
        """
        with tf.name_scope('update_value'):
            loss, summary = tf.scan(lambda _1, _2: self._update_meta_value_step(observ, reward, length),
                                    tf.range(self._config.update_epochs_value), [0., ''],
                                    parallel_iterations=1)
            print_loss = tf.Print(0, [tf.reduce_mean(loss)], 'value loss: ')
            with tf.control_dependencies([loss, print_loss]):
    ...
    def _update_meta_value_step(self, observ, reward, length):
        """Compute the current value loss and perform a gradient update step.
        ...
        Returns:
          Tuple of loss tensor and summary tensor.
        """
        loss, summary = self._meta_value_loss(observ, reward, length)
        gradients, variables = (zip(*self._value_optimizer.compute_gradients(loss)))
        optimize = self._value_optimizer.apply_gradients(zip(gradients, variables))
        summary = tf.summary.merge([
            summary,
    ...
    def _meta_value_loss(self, observ, reward, length):
        """Compute the loss function for the value baseline.
        ...
        with tf.name_scope('value_loss'):
            ...
            value_loss = 0.5 * self._mask(advantage**2, length)
            summary = tf.summary.merge([
                tf.summary.histogram('meta_value_loss', value_loss),
                tf.summary.scalar('avg_meta_value_loss', tf.reduce_mean(value_loss))
            ])
            value_loss = tf.reduce_mean(value_loss)
            return tf.check_numerics(value_loss, 'value_loss'), summary

    def _update_meta_policy():
        return xxx

    def _meta_policy_loss():
        return xxx
